...
@Suite("Volume")
struct UnitVolumeTests {

    @Test("All Volume unit properties are properly mapped")
    func allProperties() {
        let value = 1
    ...
    func edgeCases() {
        #expect(0.liters == Measurement<UnitVolume>(value: 0, unit: .liters))
        #expect((-5.0).liters == Measurement<UnitVolume>(value: -5.0, unit: .liters))
    }
}
