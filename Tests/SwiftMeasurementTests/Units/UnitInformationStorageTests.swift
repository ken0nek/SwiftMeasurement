...
@Suite("Information Storage")
struct UnitInformationStorageTests {

    @Test("All Information Storage unit properties are properly mapped")
    func allProperties() {
        let value = 1
    ...
    func edgeCases() {
        #expect(0.bytes == Measurement<UnitInformationStorage>(value: 0, unit: .bytes))
        #expect((-5.0).bytes == Measurement<UnitInformationStorage>(value: -5.0, unit: .bytes))
    }
}
